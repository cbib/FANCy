...
## removed Picrust utility functions.
## Eliminated Parallel and Delayed usage due to Permission errors.
...
def run_minpath_pipeline(inputfile,
    ...
    for sample_id in samples:
        result = minpath_wrapper(sample_id,
                                 ...
                                 out_dir, current_dir, print_cmds)
...
def identify_minpath_present(report_file):
    ...
    with open(report_file, "r") as minpath_report_in:
        for line in minpath_report_in:
            line_split = line.strip().split(None,13)
            # split lets us keep the entire pathway name intact after the 13th element ("name")
...
def parse_minpath_details(details_file, path_present):
    ...
    with open(details_file, "r") as minpath_details_in:
        for line in minpath_details_in:
            line_split = line.strip().split(None,7)
...
def minpath_wrapper(sample_id, unstrat_input, out_dir, current_dir,
                    print_opt=False):
    ...
    path2minpath = path.join(current_dir, "picrust2", 'MinPath',
                                 'MinPath12hmp.py')
